const KosCoinCrowdsale = artifacts.require("KosCoinCrowdsale");
const KosCoin = artifacts.require("KosCoin");

contract('KosCoinCrowdsale', function(accounts) {
    it('should deploy the token and store the address', function(done){
        KosCoinCrowdsale.deployed().then(async function(instance) {
    ...
    it('one ETH should buy 5 KosCoin in PreICO', function(done){
        KosCoinCrowdsale.deployed().then(async function(instance) {
            const data = await instance.sendTransaction({ from: accounts[7], value: web3.toWei(1, "ether")});
            const tokenAddress = await instance.token.call();
            const kosCoin = KosCoin.at(tokenAddress);
            const tokenAmount = await kosCoin.balanceOf(accounts[7]);
    ...
    it('one ETH should buy 2 KosCoin in ICO', function(done){
        KosCoinCrowdsale.deployed().then(async function(instance) {
            const data = await instance.sendTransaction({ from: accounts[2], value: web3.toWei(1.5, "ether")});
            const tokenAddress = await instance.token.call();
            const kosCoin = KosCoin.at(tokenAddress);
            const tokenAmount = await kosCoin.balanceOf(accounts[2]);
